...
const RESULT_EQUALITY_CLASS_NAME: &str = "com/webank/wedpr/crypto/EqualityResult";
...
pub extern "system" fn Java_com_webank_wedpr_crypto_NativeInterface_encryptMessage(
    ...
    let message = java_safe_jbytes_to_bytes!(
        _env,
        result_jobject,
        message_jbyte_array
    );
    ...
    java_safe_set_byte_array_field!(_env, result_jobject, &result.to_bytes(), "blcCipher");
...
pub extern "system" fn Java_com_webank_wedpr_crypto_NativeInterface_equalityTest(
    ...
    let cipher1_bytes = java_safe_jbytes_to_bytes!(
        _env,
        result_jobject,
        cipher1_jbyte_array
    );
    let cipher2_bytes = java_safe_jbytes_to_bytes!(
        _env,
        result_jobject,
        cipher2_jbyte_array
    );

    let cipher1_struct = match wedpr_bls12_381::WedprBls128Cipher::from_bytes(&cipher1_bytes) {
        Ok(v) => v,
        Err(_) => {return java_set_error_field_and_extract_jobject(
            &_env,
            &result_jobject,
            &format!("WedprBls128Cipher recover failed, cipher1_bytes={:?}", &cipher1_bytes),
        )},
    };
    let cipher2_struct = match wedpr_bls12_381::WedprBls128Cipher::from_bytes(&cipher2_bytes) {
        Ok(v) => v,
        Err(_) => {return java_set_error_field_and_extract_jobject(
            &_env,
            &result_jobject,
            &format!("WedprBls128Cipher recover failed, cipher2_bytes={:?}", &cipher2_bytes),
        )},
    };

    let result = wedpr_bls12_381::equality_test(&cipher1_struct, &cipher2_struct);
