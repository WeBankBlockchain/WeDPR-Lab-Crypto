...
use wedpr_l_protos::generated::ot::{
    ReceiverPublic, ReceiverSecret, SenderData, SenderDataPair, SenderPublic,
    SenderPublicPair,
};
...
pub fn sender_init(
    ...
    for data_pair in data.get_pair() {
        ...
        let encrypt_message: Vec<u8> = message
            ...
            .map(|(&x1, &x2)| x1 ^ x2)
            .collect();
        // println!("encrypt_message = {:?}", encrypt_message.clone());
...
pub fn receiver_decrypt(
    ...
    // id: &[u8],
    encrypt_message: &SenderPublic,
) -> Result<Vec<u8>, WedprError> {
    let blinding_b = bytes_to_scalar(secret.get_scalar_b())?;
    for pair in encrypt_message.get_pair() {
        // if id != pair.get_id() {
        //     continue;
        // }
...
mod tests {
    ...
    fn test_base_ot() {
        ...
        receiver_decrypt(&r_secret, &s_public).unwrap();
