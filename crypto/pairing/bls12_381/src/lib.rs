...
impl WedprBls128Cipher {
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        let result: Vec<u8> = [u_point_bytes.as_slice(), v_point_bytes.as_slice()].concat();
        return result
    }
    ...
    pub fn from_bytes(message: &[u8]) -> Result<WedprBls128Cipher, WedprError> {
        ...
        // let u_point:G1Affine = match G1Affine::from_compressed(&u_point_bytes).is_some() {
        //     Some(v) => v,
        //     None => { return Err(WedprError::FormatError) }
        // };
        if G1Affine::from_compressed(&u_point_bytes).is_some().unwrap_u8() != 1 {
            return Err(WedprError::FormatError)
        }
        let u_point:G1Affine = G1Affine::from_compressed(&u_point_bytes).unwrap();
        let mut v_point_bytes:[u8;96]  = [0; 96];
        v_point_bytes.copy_from_slice(&message[48..144]);
        println!("v1:{}", G2Affine::from_compressed(&v_point_bytes).is_some().unwrap_u8());
        if G2Affine::from_compressed(&v_point_bytes).is_some().unwrap_u8() != 1 {
            return Err(WedprError::FormatError)
        }
        let v_point:G2Affine = G2Affine::from_compressed(&v_point_bytes).unwrap();
        // let v_point:G2Affine = match G2Affine::from_compressed(&v_point_bytes).is_some() {
        //     Some(v) => v,
        //     None => { return Err(WedprError::FormatError) }
        // };
...
mod tests {
    ...
    fn test_equality() {
        ...
        let cipher1_m1_recover = WedprBls128Cipher::from_bytes(&cipher1_m1_bytes).unwrap();
